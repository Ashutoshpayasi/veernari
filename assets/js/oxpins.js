(function ($) {
      ...
      submitHandler: function (form) {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success) {
              $(form).parent().find(".result").html('<div class="alert alert-success">' + response.message + '</div>');
              $(form).find('input[type="text"], input[type="email"], textarea').val("");
            } else {
              $(form).parent().find(".result").html('<div class="alert alert-danger">' + response.message + '</div>');
            }
          },
          error: function () {
            $(form).parent().find(".result").html('<div class="alert alert-danger">An error occurred. Please try again later.</div>');
          }
        });
        return false;
      }
  ...
  for (let i = 0; i < link.length; i++) {
    link[i].addEventListener('mousemove', (e) => {
      ...
      linkHoverReveal[i].style.transform = `translate(-100%, -100% ) rotate(12deg)`;
